...
class Model:
    def __init__(self, args, logger):
        ...
        self.graves_initializer = tf.compat.v1.truncated_normal_initializer(mean=0., stddev=.075, seed=None,
                                                                            dtype=tf.float32)
        self.window_b_initializer = tf.compat.v1.truncated_normal_initializer(mean=-3.0, stddev=.25, seed=None,
                                                                              dtype=tf.float32)  # hacky initialization
        ...
        self.input_data = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.tsteps, 3])
        self.target_data = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.tsteps, 3])
        ...
        def get_phi(ascii_steps, alpha, beta, kappa):
            ...
            phi = tf.reduce_sum(input_tensor=phi_k, axis=1, keepdims=True)
            return phi  # phi ~ [?,1,ascii_steps]

        def get_window_params(i, out_cell0, kmixtures, prev_kappa, reuse=True):
            hidden = out_cell0.get_shape()[1]
            n_out = 3 * kmixtures
            with tf.compat.v1.variable_scope('window', reuse=reuse):
                window_w = tf.compat.v1.get_variable("window_w", [hidden, n_out], initializer=self.graves_initializer)
                window_b = tf.compat.v1.get_variable("window_b", [n_out], initializer=self.window_b_initializer)
            abk_hats = tf.compat.v1.nn.xw_plus_b(out_cell0, window_w, window_b)  # abk_hats ~ [?,n_out]
            abk = tf.exp(
                tf.reshape(abk_hats, [-1, 3 * kmixtures, 1]))  # abk_hats ~ [?,n_out] = "alpha, beta, kappa hats"

            alpha, beta, kappa = tf.split(abk, 3, 1)  # alpha_hat, etc ~ [?,kmixtures]
            kappa = kappa + prev_kappa
            return alpha, beta, kappa  # each ~ [?,kmixtures,1]

        self.init_kappa = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.kmixtures, 1])
        self.char_seq = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.ascii_steps, self.char_vec_len])
        ...
        with tf.compat.v1.variable_scope('mdn_dense'):
            mdn_w = tf.compat.v1.get_variable("output_w", [self.rnn_size, n_out], initializer=self.graves_initializer)
            mdn_b = tf.compat.v1.get_variable("output_b", [n_out], initializer=self.graves_initializer)

        out_cell2 = tf.reshape(tf.concat(outs_cell2, 1), [-1, args.rnn_size])  # concat outputs for efficiency
        output = tf.compat.v1.nn.xw_plus_b(out_cell2, mdn_w, mdn_b)  # data flows through dense nn
        ...
        loss = get_loss(self.pi, x1_data, x2_data, eos_data, self.mu1, self.mu2, self.sigma1, self.sigma2, self.rho,
                        self.eos)
